...
/*Modifiqu� la tabla de locales porque no ten�a un ID_Local y eso era lo que daba error en algunos procedimientos, el Primary Key
hab�a sido declarado por el nombre entonces en lugar de editar todos los procedimientos por nombre, cambie la tabla -Fernanda*/
...
/*Cuando hagan los inserts habr� un error por constraints, esto es porque se est� identificando un insert con mismo ID en 
alguno de los datos
-Fernanda*/
...
-----------PROCEDIMIENTOS DE ROLES----------------------------------
 ...
 CREATE TABLE ROLE_S(
    ...
    ROL VARCHAR2(50)
    CONSTRAINT rol_unico UNIQUE (ID_ROL) /*Le agregu� un constraint porque a la hora de querer insertar los datos, 
    deja que hayan m�s administradores, clientes o cajeros pero como no hay nada que lo diferencie, pues se ven todos repetidos. -Fernanda
    */
);
...
------------------VER ROL AC---------------------
/*No entiendo cu�l es la diferencia entre Rol AC y Rol normal, Este muestra solo un rol espec�fico basado en el ID_ROL? -Fernanda
Considerar�a Eliminar el de Ver Rol normal y dejarlo solo por ID*/
...
CREATE OR REPLACE PACKAGE Cliente_Package AS --Crea el Paquete
  PROCEDURE Obtener_Info_Cliente( --Toma los par�metros de la tabla Cliente
...
END Cliente_Package; --Cierra la especificaci�n del paquete
/

CREATE OR REPLACE PACKAGE BODY Cliente_Package AS --Crea el cuerpo del paquete, su "Relleno" pues
  PROCEDURE Obtener_Info_Cliente( --Define la implementaci�n del procedimiento Obtener_Info_Cliente, b�sicamente dice "Este procedimiento har�:..."
    p_ID_Cliente IN VARCHAR2,
    o_Nombre OUT VARCHAR2,
    o_Direccion OUT VARCHAR2,
    o_Telefono OUT VARCHAR2,
...
/ --Esto lo vi en varios videos, b�sicamente era como una forma o pr�ctica de c�digo para cerrar los procedimientos o variables PLSQL -Fernanda
-------SETS JUEGO PACKAGE -----------------------------
...
/*Ser�a bueno que revisen este porque a m� el error que me da es b�sicamente que no se encuentra o no se conecta con los datos 
de usuario que hab�a creado para el proyecto pero el procedimiento ser�a el mismo que el de los otros dos por lo que deber�a estar correcto.
-Fernanda*/
...
/*Este me dec�a que ten�a muchos datos o se insertaban muchos datos. Revisando en las tablas, en SetsJuego no existe el de "Cantidad" 
y me pas� con muchos de los procedimientos que siguen entonces solo quit� ese atributo y ya funcionaron bien. -Fernanda*/
SET SERVEROUTPUT ON;
...
/*Elimin� el otro Ver Productos porque lo sent�a repetitivo, lo dej� solo 
...
/*En este no quit� cantidad porque la variable fue seteada con descripci�n entonces no hay problema con este. -Fernanda */
...
/*Le agregué una variable más ya que solo se mostraría el nombre del cliente y puede haber más de un cliente con el mismo nombre, por eso mejor identificarlos con ID también -Fernanda */
...
--------------------TRIGGER PARA MOSTRAR LA INFORMACIÓN DEL PRODUCTO--------------------------------
/*A este también le agregué que se imprima el ID del Set del juego por lo mismo, para poder identificarlo -Fernanda*/
CREATE OR REPLACE TRIGGER Mostrar_Información_de_Producto
...
que deber�an de abrir las tablas de Sets juego con el comando SELECT * FROM SETSJUEGO y buscar los datos ya escritos, tambi�n los de las 
vistas de Ver_facturas, Eliminar_factura y esos. Hay que agregarle los datos a esos procedimientos. Ya est�n arreglados 
y elimin� algunos que consider� redundantes. -Fernanda */

 SELECT * FROM SETSJUEGO
DECLARE
    vista_factura SYS_REFCURSOR;
BEGIN
    insertar_factura(100, 15, 115, '76995', 'CLI001', 'PROD001', 115, SYSDATE, 'Direcci�n de entrega');
 
    ver_facturas(vista_factura);
    eliminar_factura('76995');
    actualizar_factura('76995', 120, SYSDATE, 'Nueva direcci�n de entrega');
END;
